...
public class TiledBackground extends GraphicObject {
	...
	public TiledBackground(ExtendedImage tilePixels, ExtendedImage tileMask, byte[] map, int widthInTiles,
			int heightInTiles) {
	...
	public TiledBackground(Image tilePixels, Image tileMask, byte[] map, int widthInTiles, int heightInTiles) {
		System.out.println(
				"public TiledBackground(Image tilePixels, Image tileMask, byte[] map, int " + widthInTiles + ", int " +
						heightInTiles + ")");
	...
	protected void paint(Graphics g) {
		for (int y = posy / 8; y < heightInTiles; y++) {
			for (int x = posx / 8; x < widthInTiles; x++) {
				if (-posx + x * 8 > DeviceFactory.getDevice().getDeviceDisplay().getWidth())
					break;
				g.drawImage(pixels[map[y * widthInTiles + x]], -posx + x * 8, -posy + y * 8, 0);
			}
			if (-posy + y * 8 > DeviceFactory.getDevice().getDeviceDisplay().getHeight())
				break;
		}
	}
}
